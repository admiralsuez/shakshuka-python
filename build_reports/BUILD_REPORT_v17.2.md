# Build Report - Shakshuka v17.2-b119

## Build Information
- **Version:** 17.2
- **Build Number:** 119
- **Build Date:** 2025-12-25 04:36:30
...
### 2. Installer Package
- **Filename:** `Shakshuka-Setup-v17.2.exe`
- **Size:** 30.27 MB
...
- **Installer:** `C:\Users\vibin\OneDrive\Desktop\shakshuka-python-linux_stage1\Shakshuka-Setup-v17.2.exe`
...
1. Run `Shakshuka-Setup-v17.2-b119.exe`
...
Release Date: 2025-12-25T04:35:41.436354

Highlights

---
## Version 17.1 - Release
Release Date: 2025-12-25T03:22:56.429015

Highlights

---
## Version 17.0 - Release
Release Date: 2025-12-25T03:22:01.809210

Highlights

---
## Version 16.9 - Release
Release Date: 2025-12-25T00:19:01.984731

Highlights

---
## Version 16.8 - Release
Release Date: 2025-12-24T01:21:39.457142
...
---
## Version 16.0 - Performance & Architecture Improvements
Release Date: 2025-12-21T01:30:00.000000

Highlights

- **Performance**: Created consolidated `/api/analytics/summary` endpoint that returns all dashboard data in a single API call, reducing 5-6 separate requests to 1 (60-70% reduction in API calls).
- **Performance**: Optimized `get_daily_recap` database query - replaced N+1 query pattern (6 separate COUNT queries) with single query using subqueries, improving daily recap load time.
- **Performance**: Updated `dashboard.js` to use consolidated analytics endpoint with 5-second caching, eliminating redundant API calls on page load.
- **Error Handling**: Created centralized `ErrorHandler` class with graceful degradation - provides `criticalOperation()`, `withGracefulDegradation()`, and `fetchWithFallback()` helpers.
- **Error Handling**: Wrapped strike operations (`strikeTaskToday`, `strikeTaskForever`) with `criticalOperation()` wrapper for proper error handling and user feedback.
- **Error Handling**: Added fallback values and retry logic for failed API calls - app continues working even when backend calls fail.
- **Modal Management**: Created centralized `ModalManager` class to handle all modal operations - provides unified `open()`, `close()`, `closeAll()`, `toggle()` methods.
- **Modal Management**: Added ESC key support, backdrop click-to-close, and auto-focus for all modals through ModalManager.
- **Code Quality**: Fixed expired completed tasks showing in expired section by updating `isExpired()` helper to exclude completed/struck_forever tasks.

Technical Notes
- Created `modal-manager.js` with backward-compatible global `openModal()`/`closeModal()` functions.
- Created `error-handler.js` with built-in request cache (TTL-based) and error logging.
- New `/api/analytics/summary` endpoint returns: tasks stats, strikes, streak, completed_forever, settings_changes, tasks_added, tasks_retried.
- `get_daily_recap()` now uses single SQL query with 6 subqueries instead of 6 separate queries.
- `dashboard.js` uses `fetchWithFallback()` with 5-second cache TTL and graceful degradation.
- Strike operations now show proper error messages instead of silent failures.
- Both new core modules loaded early in scripts.html before other dependencies.

---
## Version 15.9 - Daily Recap & Task Retry Features
Release Date: 2025-12-21T00:00:00.000000

Highlights

- **Daily Recap Modal**: Fixed daily recap modal not appearing - modal was in wrong template file (index.html vs index_modular.html). Created dedicated modal partial and added to modular template structure.
- **Daily Recap Modal**: Redesigned UI to remove large points display, showing only streak banner and stats grid for cleaner presentation.
- **Daily Recap Modal**: Reduced top padding to eliminate empty space above streak banner.
- **Daily Recap Modal**: Added "Tasks Retried" metric to recap stats.
- **Task Retry**: Added retry functionality for expired tasks - clicking retry button opens simple date picker modal to reschedule task to future date.
- **Task Retry**: Expired tasks now show both retry (🔄) and strike (✓) buttons, allowing users to either reschedule or complete expired tasks.
- **Task Retry**: Date picker prevents selecting past dates (min set to today) and defaults to tomorrow.
- **Task Retry**: Fixed "Missing required field: title" error by sending complete task data in PUT request.
- **Analytics**: Added "Tasks Retried" card to analytics page (currently shows 0, backend tracking to be implemented).
- **Analytics**: Removed "Recent Tasks" and "Quick Actions" sections from analytics page for cleaner layout.
- **Analytics**: Fixed streak calculation - now fetches from backend API instead of local calculation to match recap modal's value.
- **Navigation**: Reordered sidebar navigation - Notes now appears above Analytics.
- **Strike Calendar**: Reverted visual scale changes to original size.

Technical Notes
- Created `retry_modal.html` partial with simple date-only picker (no task details, project, time fields).
- Created `retry-task.js` to handle retry functionality with date validation and complete task data preservation.
- Created `daily_recap_modal.html` partial in modals directory.
- Updated `dashboard.js` to fetch streak from `/api/analytics/daily-recap` endpoint for consistency.
- Modal element querying now uses `_ensureModal()` helper to handle SPA timing issues.
- Added console logging to daily recap functions for debugging modal visibility issues.

---
## Version 15.8 - Release
Release Date: 2025-12-20T16:52:55.852366

Highlights

---
## Version 15.7 - Release
Release Date: 2025-12-20T00:49:37.670826

Highlights

---
## Version 15.6 - Release
Release Date: 2025-12-19T13:23:10.064366

Highlights

---
## Version 15.5 - Release
Release Date: 2025-12-18T23:37:42.265381

Highlights

---
## Version 15.4 - Release
Release Date: 2025-12-18T23:30:38.600144

Highlights

---
## Version 15.3 - Release
Release Date: 2025-12-18T12:43:42.300415

Highlights

---
## Version 15.2 - Release
Release Date: 2025-12-18T12:14:32.563825

Highlights

- Analytics: fixed Analytics page cards (Completed Today / Day Streak / Striked Today) so they refresh reliably using current task state and `/api/analytics`.
- Analytics: "Completed Forever" now counts tasks completed normally as well as tasks completed forever.
- Changelog: consolidated consecutive versions with empty Highlights into version ranges (e.g., 14.4–14.9) for a cleaner changelog.
- UI: Matte finish now applies a visible matte look across buttons.
- Updates/Installer: bumped installer version to 15.2 and improved upgrade shutdown via `Shakshuka.exe --shutdown` with fallback force-close.

---
## Versions 15.0 – 15.1 - Release
Release Period: 2025-12-18T00:09:03.984815 – 2025-12-18T01:15:08.964109

Highlights

- Settings: autosave interval now saves via `PUT /api/settings` (`autosave_interval`) instead of a missing `/api/settings/autosave` endpoint.
- Data Management: added `GET /api/export` (downloads tasks, notes, settings, planner schedule/history) and `POST /api/clear` (clears all user data) so Settings buttons work reliably.
- Notes: split view now reliably supports opening a different note in the secondary pane without overwriting the primary note's secondary content.
- Notes: split-pane chooser now prompts Primary (Left) vs Secondary (Right) with note titles so it's clear what will be shown/replaced.
- Analytics: "Completed Forever" counter now reflects forever-completed tasks correctly.
- UI: Matte finish now visibly applies a matte look across buttons.
- Updates/Installer: added `Shakshuka.exe --shutdown` to help installers close the running app; it attempts graceful shutdown then falls back to force-closing if needed.

---
## Versions 14.4 – 14.9 - Frontend Asset Reorganization
Release Period: 2025-12-16T02:16:00.000000 – 2025-12-17T15:44:14.830372

Highlights
- Frontend: reorganized static JS and CSS into dedicated subfolders under `assets/static/js/` and `assets/static/css/` for clearer modular structure.
- Frontend: updated all template `<script>` / `<link>` references and `style.css` `@import` paths to match the new folder layout.
- Fix: legacy `index.html` now includes `partials/scripts.html` to ensure consistent script load order and prevent missing-global runtime errors (e.g. `openLogsModal`, `canStrikeTask`, `loadUpdateSettings`).

Technical Notes
- CSS: `assets/static/css/style.css` is now imports-only, preserving cascade order while splitting modules into `core/`, `layout/`, `components/`, `pages/`, `features/`, and `auth/`.
- JS: scripts now load from `auth/`, `core/`, `utils/`, `pages/`, and `app/` subfolders while preserving the original boot sequence.

---
## Versions 14.0 – 14.3 - Release
Release Period: 2025-12-12T00:31:47.548283 – 2025-12-12T01:30:59.973729
...
---
## Versions 13.8 – 13.9 - Release
Release Period: 2025-12-11T23:50:21.466966 – 2025-12-11T23:57:14.570864

Highlights
- Updates: GitHub update modal now shows a progress bar while the installer is downloading so you always see activity.
- Updates: GitHub installer downloads are now saved into your Downloads folder (instead of a hidden temp file) so you can easily run them.
- Updates: Wording in the GitHub update flow clearly explains that Shakshuka will not auto-restart; close the app and run the downloaded installer manually.
...
---
## Versions 13.0 – 13.4 - Notes Workspace & Task UX
Release Period: 2025-12-10T10:52:27.000000 – 2025-12-11T00:45:03.116133

Highlights
- New Notes workspace with multiple tabs, split view, and local auto-save so you can keep quick scratch notes inside Shakshuka.
- Notes sidebar nav item and "P" keyboard shortcut to jump into Notes from anywhere.
- Notes tabs are closable and renameable (double-click the tab title or use the View Notes list), with improved hover/focus states.
- Slash ("/") command menu in Notes for quick line formatting (bullet list, numbered list with auto-continue, and large header) plus a Clear option.
- Tab key in Notes now indents/outdents selections; bullet and numbered lists auto-continue on Enter and exit cleanly on double Enter.
- "Make selection a task" in Notes turns highlighted text into a task with confirmation, and task filters/notifications stay in sync.
- "Import Tasks" moved into the Add Task options modal; the old header Import button and sidebar nav entry were removed for a cleaner layout.
- Task/project filters and toast notifications refined (bottom-right stacking, short lifetime) to better match the current workflow.

---
## Versions 11.0 – 12.1 – SQLite Daily Strikes & Build Iterations
...
- Tasks: due date pills respect Casual Dates (today, tomorrow, this weekend) and highlight today's items while grouping them at the top.
- Rendering: task list rendering scheduled via `requestAnimationFrame` (with a timeout fallback) to smooth out rapid filter/strike updates.
