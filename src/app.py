...
def initialize_data_manager():
    ...
    try:
        ...
        try:
            ...
            um_data_dir = get_user_data_dir()
            os.makedirs(um_data_dir, exist_ok=True)

            app_context.update_manager = UpdateManager(app_dir=app_root, data_dir=um_data_dir)
...
def update_task(task_id):
    ...
    try:
        ...
        if success:
            ...
            updated_task = app_context.data_manager.get_task_by_id(user_id, task_id)
            if updated_task:
...
def delete_task(task_id):
    ...
    try:
        ...
        task_to_delete = app_context.data_manager.get_task_by_id(user_id, task_id)
...
def complete_task(task_id):
    ...
    for i, task in enumerate(tasks):
        if task['id'] == task_id:
            ...
            tasks[i]['completed_at'] = datetime.utcnow().isoformat()  # UTC for consistency
...
def unschedule_task(task_id):
    ...
    # Validate task_id format (should be a non-empty string)
    ...
    try:
        ...
        for i, task in enumerate(tasks):
            if task['id'] == task_id:
                ...
                tasks[i]['scheduled_minute'] = None  # Clear minute
...
def schedule_task(task_id):
    ...
    minute = schedule_data.get('minute', 0)  # Default to 0 minutes
    ...
    for task in tasks:
        ...
        if (start_minutes < task_end_minutes and end_minutes > task_start_minutes):
            # Get task start time for the error message
    ...
    for i, task in enumerate(tasks):
        if task['id'] == task_id:
            ...
            tasks[i]['scheduled_minute'] = minute  # Store minute
